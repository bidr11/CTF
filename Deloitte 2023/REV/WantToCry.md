#### 100 points Want to cry

The successful WannaCry ransomware was released on the 12th of May 2017.  
Our honeypots identified another malware family released on this date, however it was much more targeted and less widely spread  
We extracted the module responsible for command and control, but cannot identify the domain..  
Can you find it for us?  
Flag format: URL (for example hackazon.org)

```c
#include <stdio.h>
#include <time.h>

int main() {
	int val;

	time_t time_var= 1494547200; // UNIX Timestamp for 12 May 2017
	struct tm *localtime_var;
	int day;
	int mon;
	int year;

	localtime_var = localtime(&time_var);
	day = localtime_var->tm_mday;
	mon = localtime_var->tm_mon + 1;
	year = localtime_var->tm_year + 0x76c;

	printf("%d %d %d\n", day, mon, year);

	for (int i = 0; i< 7; i++) {
		val = i + (((year - day) + day * day + mon * i) - mon) + 2;
		printf("%d\n", val);
	}
	return 0;
}

```

## Solution
```python
mem = [ 0x7e, 0x42, 0xa0, 0x15, 0x25, 0x2a, 0xef]

def process_mem(mem):

	initial = [0]*7

	initial[0] = 0x16;
	initial[1] = 0x36;
	initial[2] = 0xd4;
	initial[3] = 0x65;

	initial[4] = 0x1f;
	initial[5] = 5;
	initial[6] = 0xc0;

	counter = 0;

	while( True ) :

		mem_len = len(mem)
		if (mem_len <= counter):
			break;

		intermediary = (counter >> 0x1f) >> 0x1d;
		mem[counter] = initial[(counter + intermediary & 7) - intermediary] ^ mem[counter]

		counter = counter + 1;

	return "".join([chr(i) for i in mem])

flag = ""
flag += process_mem(mem)

year = 2017
day = 11
mon = 5
l=[]
for i in range(7):
	l.append( i + (((year - day) + day * day + mon * i) - mon) + 2)
	
indices = [i%0x6a for i in l]

array_ptrs = b''
array_ptrs = array_ptrs.split(b"\x00")
array_ptrs = [i.decode() for i in array_ptrs if i]

for i in indices:
	flag += array_ptrs[i]

mem3 = [ 0x38, 0x5a, 0xbb, 0x06, 0x7e, 0x69 ]

flag += process_mem(mem3)

print(flag)
```

## Flag
`http://abraxasagenaalconbombercreeperinit1984mdef.local`